...
export const downloadGradeSheet = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const isOwner = String((attempt.student as { _id: unknown })._id || attempt.student) === req.user!.id;
  if (!isOwner && req.user!.role === 'student') throw new ApiError(403, 'Access denied');

  const exam = await Exam.findById(attempt.exam);
  const questions = await Question.find({ _id: { $in: exam?.questions || [] } });

  const student = attempt.student as unknown as { name: string; email: string };
  const examData = attempt.exam as unknown as { title: string; totalMarks: number; passingMarks: number };

  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="grade-sheet-${attempt._id}.pdf"`
  );
