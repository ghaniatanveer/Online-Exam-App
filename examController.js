...
export const createExam = asyncHandler(async (req, res) => {
    const { title, description, duration, passingMarks, startTime, endTime, questionIds, batchIds, status, selectionMode, shuffleQuestions, shuffleOptions, allowReview, } = req.body;
...
export const updateExam = asyncHandler(async (req, res) => {
    ...
    const exam = await Exam.findOneAndUpdate({ _id: req.params.id, createdBy: req.user.id }, updates, { new: true });
    if (!exam)
        throw new ApiError(404, 'Exam not found');
    res.json({ success: true, data: exam });
});
