...
async function studentCanAccessExam(examId, studentId) {
    ...
    if (user?.batch)
        batchIds.push(String(user.batch));
...
export const startAttempt = asyncHandler(async (req, res) => {
    ...
    const expiresAt = new Date(Math.min(now.getTime() + exam.duration * 60 * 1000, exam.endTime.getTime()));
...
export const saveAnswer = asyncHandler(async (req, res) => {
    ...
    if (!attempt)
        throw new ApiError(404, 'No active attempt');
...
export const reportTabSwitch = asyncHandler(async (req, res) => {
    const attempt = await ExamAttempt.findOneAndUpdate({ exam: req.params.examId, student: req.user.id, status: 'in_progress' }, { $inc: { tabSwitchCount: 1 } }, { new: true });
    if (!attempt)
        throw new ApiError(404, 'No active attempt');
...
export const submitAttempt = asyncHandler(async (req, res) => {
    ...
    for (const question of questions) {
        const answerIdx = attempt.answers.findIndex((a) => String(a.questionId) === String(question._id));
